...
export function useEndpoints() {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'checks' }, ({ new: c }: { new: { endpoint_id: string; is_up: boolean; response_time_ms: number | null; checked_at: string } }) => {
  ...
  const addEndpoint = useCallback(async (form: AddEndpointForm): Promise<void> => {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        interval_seconds: form.interval_seconds, expected_status: form.expected_status,
        timeout_ms: form.timeout_ms, slo_target: form.slo_target,
      })
      .select().single()
    if (error) throw error
    dispatch({ type: 'ADD_ENDPOINT', payload: { ...(data as Endpoint), status: 'unknown' } })
    await logAudit({ action: 'endpoint.created', resourceType: 'endpoint', resourceId: (data as Endpoint).id, resourceName: form.name })
  }, [])
  ...
  const removeEndpoint = useCallback(async (id: string): Promise<void> => {
    ...
    await logAudit({ action: 'endpoint.deleted', resourceType: 'endpoint', resourceId: id, resourceName: ep?.name })
  }, [state.endpoints])
